'''
Embedding evaluation script for COMS W4705 Fall 2021 HW3.

You can run this script from the command-line on one file at a time using
...
def collect(model):
    ...
    if type(model) is dict:
        ...
        vocab = [k for k in model.key_to_index.keys()]
...
def eval_bats_file(
    model, matrix, transposed, vocab, indices, f, repeat=False, multi=0):
    '''
    ...
    for dr in os.listdir('data/BATS'):
        if os.path.isdir(os.path.join(base, dr)):
            ...
            for f in os.listdir(os.path.join(base, dr)):
                accs[f.split('.')[0]] = eval_bats_file(
                    model, matrix, transposed, vocab, indices, os.path.join(base, dr, f))
...
def eval_msr(model):
    ...
    #print(tr_cos)
    #print(np.any(np.isnan(tr_cos)))
    #print(np.all(np.isfinite(tr_cos)))
